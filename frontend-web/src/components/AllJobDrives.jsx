...
const parseRoundsFromValue = (roundsValue) => {
  if (!roundsValue) {
    return [];
  }
  ...
  if (typeof roundsValue === "string") {
    ...
    return normalized
      .split(",")
      .map((round) => round.replace(/^"|"$/g, "").trim())
      .filter(Boolean);
  }
...
const buildFallbackSelectionRounds = (roundsValue) =>
  parseRoundsFromValue(roundsValue).map((roundName) => ({
    name: roundName,
    details: "",
    date: null,
    time: null,
...
const AllJobDrives = () => {
  ...
  const getDriveSelectionRounds = (drive) => {
    if (Array.isArray(drive?.selectionRounds) && drive.selectionRounds.length > 0) {
      return drive.selectionRounds;
    }

    return buildFallbackSelectionRounds(drive?.rounds);
  };

  const getRoundStudentCount = (drive, round) => {
    const selectedCount = Array.isArray(round?.selectedStudents)
      ? round.selectedStudents.length
      : 0;
    ...
    if (round?.fromRoundsColumn) {
      return getAppliedStudentsCount(drive);
    }
  ...
  const canManageDrive = (drive) => {
    ...
    if (normalizedRole === "po" || normalizedRole === "placement_officer") {
      return true;
    }
    ...
    if (drive.spocDept) {
      const spocDeptNormalized = normalizeDepartment(drive.spocDept);
      return Boolean(userDept && spocDeptNormalized && userDept === spocDeptNormalized);
    }
    ...
    if (!drive.createdBy?._id && userDept) {
  ...
  const isApplicationDeadlinePassed = (drive) => {
    ...
    // Consider applications open until end of deadline day
    // drive.time is now strictly Drive Time, not deadline time
  ...
  // Helper to check if a round is currently active for management
  const isRoundActive = (drive, roundIndex) => {
    // 1. Global check: If drive has ended, no rounds are active
    if (isDriveEnded(drive)) return false;

    const rounds = drive.selectionRounds;
    if (!rounds || !rounds[roundIndex]) return false;

    const currentRound = rounds[roundIndex];
    const now = new Date();

    // Determine the start of the management window for this round
    let managementWindowStart;
    if (roundIndex === 0) {
      // For the first round, management starts after the application deadline
      if (!drive.deadline) return false; // Cannot determine start without deadline
      managementWindowStart = new Date(drive.deadline);
      if (drive.applicationDeadlineTime) {
        const [h, m] = drive.applicationDeadlineTime.split(":");
        managementWindowStart.setHours(parseInt(h), parseInt(m), 59, 999);
      } else {
        managementWindowStart.setHours(23, 59, 59, 999); // End of day
      }
    } else {
      // For subsequent rounds, management starts when the round starts
      if (!currentRound.date) return false; // Not scheduled
      managementWindowStart = new Date(currentRound.date);
      if (currentRound.time) {
        const [h, m] = currentRound.time.split(":");
        managementWindowStart.setHours(parseInt(h), parseInt(m), 0, 0);
      } else {
        managementWindowStart.setHours(0, 0, 0, 0);
      }
    }

    // 2. Check if the management window has started
    if (now < managementWindowStart) return false;

    // 3. Check if the management window has ended (i.e., next round started)
    if (roundIndex < rounds.length - 1) {
      const nextRound = rounds[roundIndex + 1];
      if (nextRound && nextRound.date) {
        const nextRoundStart = new Date(nextRound.date);
        if (nextRound.time) {
          const [h, m] = nextRound.time.split(":");
          nextRoundStart.setHours(parseInt(h), parseInt(m), 0, 0);
        } else {
          nextRoundStart.setHours(0, 0, 0, 0);
        }
        if (now >= nextRoundStart) return false;
      }
    }
  ...
  const handleViewPlacedStudents = async (drive) => {
    try {
      ...
      if (driveEnded && rounds.length > 0) {
        ...
        const selectedIds = Array.isArray(lastRound?.selectedStudents)
          ? lastRound.selectedStudents
          : [];

        if (selectedIds.length > 0) {
          const byIdsResponse = await fetch(
            `${API_BASE}/api/job-drives/${drive._id}/get-students-by-ids`,
            {
              method: "POST",
              headers: {
                Authorization: `Bearer ${token}`,
          ...
          if (byIdsResponse.ok) {
            const byIdsData = await byIdsResponse.json();
            if (byIdsData.students && byIdsData.students.length > 0) {
              const mappedStudents = byIdsData.students.map((student) => ({
                name: student.profile?.name || student.name || "N/A",
                rollNumber: student.profile?.rollNumber || student.rollNumber || "N/A",
                department: student.profile?.department || student.department || "N/A",
                cgpa: student.profile?.cgpa || student.cgpa || "N/A",
        ...
        if (applicantsResponse.ok) {
          ...
          if (applicantsData.students && applicantsData.students.length > 0) {
            const fallbackStudents = applicantsData.students.map((student) => ({
              name: student.name || student.profile?.name || "N/A",
              rollNumber: student.rollNumber || student.profile?.rollNumber || "N/A",
              department: student.department || student.profile?.department || "N/A",
              cgpa: student.cgpa || student.profile?.cgpa || "N/A",
              email: student.email || "N/A",
              mobileNumber:
                student.mobileNumber ||
                student.profile?.mobileNumber ||
            ...
            const driveWithFallbackStudents = {
              ...drive,
              placedStudents: fallbackStudents,
            };

            setSelectedDriveForView(driveWithFallbackStudents);
            setShowPlacedStudentsViewModal(true);
            return;
          }
        }
  ...
  const fetchAllJobDrives = async () => {
    try {
      ...
      const normalizedDrives = (response.data.jobDrives || []).map((drive) => {
        const hasSelectionRounds =
          Array.isArray(drive.selectionRounds) && drive.selectionRounds.length > 0;

        if (hasSelectionRounds) {
          return drive;
        }

        return {
          ...drive,
          selectionRounds: buildFallbackSelectionRounds(drive.rounds),
        };
      });
  ...
  // Add useEffect to refetch when department filter changes
  useEffect(() => {
    fetchAllJobDrives();
  }, [departmentFilter]);
  ...
  const handleRoundSelect = async (round, roundIndex) => {
    ...
    try {
      ...
      const response = await axios.get(
        ...
        const rounds = getDriveSelectionRounds(selectedDriveForRounds);
        ...
        {filteredDrives.length === 0 ? (
          ...
          <div className="grid gap-6">
            {filteredDrives.map((drive) => {
              const displayRounds = getDriveSelectionRounds(drive);
              const driveEnded = isDriveEnded(drive);
              const hasSelectionRounds =
                Array.isArray(displayRounds) &&
                ...
                <div key={drive._id} className="bg-white p-6 rounded-lg shadow">
                          ...
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <span className="text-sm text-gray-600 font-medium">
                                ({getRoundStudentCount(drive, round)} selected)
                              </span>
                            </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-200">
                    <div className="text-sm text-gray-500">
                      ...
                      <p>
                        Placed Students: {getPlacedStudentsCount(drive)}
                        {displayRounds.length > 0 && isDriveEnded(drive) && (
                      ...
                      {selectedDriveForView.placedStudents.map(
                        (student, index) => (
                          <tr key={index} className="hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3 text-sm">{student.cgpa || "N/A"}</td>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {getDriveSelectionRounds(selectedDriveForRounds).map((round, index) => {
                    const isActive = isRoundActive(selectedDriveForRounds, index);
                    return (
                      <div
                        key={index}
                        ...
                        onClick={() => isActive && handleRoundSelect(round, index)}
                      >
                        <div className="flex justify-between items-center">
...
const RoundStudentsModal = ({
  ...
  const fetchSelectedStudents = async () => {
    try {
      ...
      let response;
      if (Array.isArray(round.selectedStudents) && round.selectedStudents.length > 0) {
  ...
  const downloadRoundStudentsCSV = () => {
    if (!students || students.length === 0) {
      toast.error("No data to download");
      return;
    }

    const headers = ["S.No", "Name", "Roll Number", "Department", "Email", "Mobile", "CGPA"];  
    const csvContent = [
      headers.join(","),
      ...students.map((student, index) =>
        [
          ...
          `"${student.profile?.rollNumber || student.rollNumber || ""}"`,
          `"${student.profile?.department || student.department || "N/A"}"`,
          `"${student.email || ""}"`,
          `"${student.profile?.mobileNumber || student.mobileNumber || student.mobile || student.phone || student.profile?.phoneNumber || "N/A"}"`,
    ...
    link.setAttribute(
      "download",
      `${jobDrive.companyName}_${round.name || `round_${roundIndex}`}_selected_students.csv`
    );
    link.style.visibility = "hidden";
    document.body.appendChild(link);
    link.click();
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-5xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="mb-4 p-3 bg-blue-50 rounded-lg">
          <p className="text-sm text-blue-700">
            ...
            <strong> Selected:</strong> {round.selectedStudents?.length || students.length || 0}{" "}
            students
          </p>
        </div>
              ...
              <tbody className="divide-y divide-gray-200">
                {students.map((student, index) => (
                  <tr key={student._id || index} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm">
                      {student.profile?.phoneNumber || student.mobileNumber || student.mobile || student.phone || student.profile?.mobileNumber || "N/A"}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="mt-6 flex justify-between items-center">
          {fromPR ? (
            <button
              onClick={downloadRoundStudentsCSV}
              className="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 flex items-center space-x-2"
            >
              <svg
                className="w-4 h-4"
...
// d:\College Placement Application\College-Placement-Application\frontend-web\src\components\AllJobDrives.jsx
